...
export function AnimatedGlowButton<T extends ElementType = "button">({
  ...
  isDisabled = false,
  ...props
}: AnimatedGlowButtonProps<T> &
  Omit<ComponentPropsWithoutRef<T>, keyof AnimatedGlowButtonProps<T>>) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={cn(
              "relative flex w-full items-center overflow-hidden rounded-xl border border-border bg-card/90 p-4 backdrop-blur-xs",
              className,
            )}
          >
            <motion.div
            ...
            <div
              className={`relative z-10 mr-4 flex h-12 w-12 items-center justify-center rounded-lg ${colors.bg}`}
            >
              <motion.div
                className={`absolute inset-0 rounded-lg ${colors.bgGlow} opacity-70 blur-md`}
                animate={{
                  opacity: isAnimationActive
                    ? [0.4, 0.7, 0.4]
                    : [0.3, 0.5, 0.3],
              ...
              >
                {icon || (
                  <Shield className="relative z-10 h-6 w-6 text-white" />
                )}
              </motion.div>
            </div>
